...
async function assistant(req,res){
  ...
  let raw='';
  req.on('data',c=>{raw+=c;if(raw.length>120000)req.destroy()});
...
const steps=stepNodes.map(x=>{const d=x.querySelector('div:last-child');return d?d.innerText.replace(/^Bước\\s+\\d+/i,'').trim():x.innerText.trim()}).filter(Boolean);
...
document.getElementById('voiceClose').onclick=()=>{panel.classList.remove('on');document.body.style.overflow='';if('speechSynthesis'in window)speechSynthesis.cancel()};
document.getElementById('voiceNext').onclick=next;document.getElementById('voicePrev').onclick=prev;
...
http.ServerResponse.prototype.end=function(chunk,encoding,cb){
  try{
    ...
    const isRecipe=/^\/cong-thuc\/[^/?#]+\/?(?:\?.*)?$/.test(url);
    const ct=String(this.getHeader&&this.getHeader('Content-Type')||'');
    if(isRecipe&&typeof chunk==='string'&&chunk.includes('</body>')&&!chunk.includes('id="voiceLaunch"')&&(ct.includes('text/html')||ct==='')){
      chunk=chunk.replace('</body>',addon+'</body>');
      if(this.removeHeader)this.removeHeader('Content-Length');
    }
  }catch(e){console.error('[Voice inject]',e.message)}
  return nativeEnd.call(this,chunk,encoding,cb)
};
